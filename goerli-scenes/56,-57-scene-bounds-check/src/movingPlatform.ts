...
export function setAsMovingPlatform(
  entity: Entity,
  waypoints: Vector3[],
  travelDurationInSeconds: number
) {
...
export function platformsMovementSystem(deltaTime: number) {
  for (const [entity] of engine.getEntitiesWith(MovingPlatform)) {
    ...
    movingPlatformComp.currentLerpTimeBetweenWaypoints +=
      movingPlatformComp.speed * deltaTime
...
function calculateMovementSpeed(
  ...
  for (let i = 0; i < waypoints.length - 1; i++) {
    totalPathMagnitude += Vector3.length(
      Vector3.subtract(waypoints[i + 1], waypoints[i])
    )
  }
