...
export function createInputSystem(engine: IEngine) {
  ...
  function* getLastTriggered(): Generator<
    [Entity, DeepReadonlyObject<PBPointerEventsResult>]
  > {
    for (const [entity, valueSet] of engine.getEntitiesWith(
      PointerEventsResult
    )) {
  ...
  function findClick(inputAction: InputAction, entity: Entity) {
    ...
    if (
      ...
      timestampIsCurrentFrame(up.timestamp)
    ) {
      return { up, down }
    }
  ...
  function getInputCommand(
    ...
    if (entity) {
      ...
      for (const command of globalState.thisFrameCommands) {
        if (
          (command.button === inputAction ||
            inputAction === InputAction.IA_ANY) &&
          command.state === pointerEventType
        ) {
          return command
        }
  ...
  function isPressed(inputAction: InputAction) {
    return (
      globalState.buttonState.get(inputAction)?.state ===
