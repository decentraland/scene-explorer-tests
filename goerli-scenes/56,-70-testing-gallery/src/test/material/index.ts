...
export function testMaterial(position: Vector3) {
  ...
  cubeWithMaterialTimer.add((index) => {
    ...
    post.displayText(
      `[${index}] Albedo=(0,0,1) Emissive=(0,0,1) EmissiveIntensity=100`
    )
  ...
  cubeWithMaterialTimer.add((index) => {
    ...
    post.displayText(
      `[${index}] Albedo=(0,0,1) Metallic=0 Roughness=0 Alpha=0.5`
    )
    post.displayImage(`src/test/material/material-${index}.png`)
  })
  ...
  cubeWithMaterialTimer.add((index) => {
    Material.setPbrMaterial(entity, {
      ...
      bumpTexture: {
        tex: {
          $case: 'texture',
          texture: { src: 'src/test/material/normal_mapping_normal_map.png' }
        }
      },
      albedoColor: Color4.Blue()
    })
    post.displayText(
      `[${index}] Albedo=(0,0,1) Metallic=0.5 Roughness=0.2 Alpha=1 NormalMap`
    )
    post.displayImage(`src/test/material/material-${index}.png`)
  })
  ...
  cubeWithMaterialTimer.add((index) => {
    ...
    post.displayText(
      `[${index}] Albedo=(0,0,1) Metallic=0.5 Roughness=0.5 Alpha=1`
    )
    post.displayImage(`src/test/material/material-${index}.png`)
  })
