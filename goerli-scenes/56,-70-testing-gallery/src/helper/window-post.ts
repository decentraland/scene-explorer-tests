...
const callbacks: Map<
  Entity,
  { onPauseResume?: () => void; onPrevious?: () => void; onNext?: () => void }
> = new Map()
...
export function createPost(
  position: Vector3,
  options: Partial<PostOption> = {}
) {
  const { windowSize, thickness, backgroundColor, fontColor, fontSize, title } =
    { ...defaultPostOption, ...options }
  ...
  Transform.create(extraInfoEntity, {
    position: Vector3.create(0, windowSize.y - 0.5, 0),
    parent
  })
  TextShape.create(extraInfoEntity, {
    text: title,
    fontSize: 1,
  ...
  createBox(
    ...
    Vector3.create(windowSize.x + 0.1, windowSize.y + 0.1, thickness / 4),
    parent
  )
  createBox(
    Vector3.create(0, 0, 0),
    Vector3.create(windowSize.x, windowSize.y, thickness / 2),
    parent,
    backgroundColor
  )
  ...
  Transform.create(colliderEntity, {
    scale: Vector3.create(windowSize.x, windowSize.y, thickness),
    parent
  })
  ...
  Transform.create(labelEntity, {
    position: Vector3.create(0, 0, -thickness / 2),
    parent
  })
  ...
  return {
    rotate(angle: number) {
      ...
      transform.rotation = Quaternion.multiply(
        transform.rotation,
        Quaternion.fromAngleAxis(angle, Vector3.Up())
      )
      return this
    },
    move(v: Vector3) {
...
export function postColliderSystem() {
  for (const [entity, value] of customInputSystem.getLastTriggered()) {
    ...
    if (cbs && value.state == 1) {
      if (
        cbs.onNext &&
        value.button === InputAction.IA_ACTION_4 &&
        value.state == 1
      ) {
        cbs.onNext()
      } else if (
        cbs.onPrevious &&
        value.button === InputAction.IA_ACTION_3 &&
        value.state == 1
      ) {
        cbs.onPrevious()
      } else if (
        cbs.onPauseResume &&
        value.button === InputAction.IA_PRIMARY &&
        value.state == 1
      ) {
        cbs.onPauseResume()
      }
